...
public class Baseline extends JobExcecutorBase {
	...
	public void run() {
		...
		try {
			...
			// If not specified, use RandomQuerySelector
	...
	private void runOnTrainData(Classifier base, IntfQuerySelector qs, Instances trainData, int cIdx, int numFolds, int seed, double budget, String delim, String outFile) throws Exception{
		...
		double baseErrorTrain=0;
		double baseErrorTest=0;
		for (int i=0; i<numFolds; i++){
			...
			baseCopy.buildClassifier(train);
			baseErrorTrain+= errorRate(train, baseCopy);
			baseErrorTest+=  errorRate(test,baseCopy);
		}
		baseErrorTrain= baseErrorTrain/numFolds;
		baseErrorTest= baseErrorTest/numFolds;
		
		System.out.println("1,0.0,"+baseErrorTrain+","+baseErrorTest);
	...
	private double errorRate(Instances pool, Classifier c) throws Exception {

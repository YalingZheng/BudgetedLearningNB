...
public class AbsoluteBR2QuerySelector extends QuerySelector {
...
//	private int counter=0;
	...
	public Enumeration listOptions() {
	    ...
	    newVector.addElement(new Option(
		      "\talpha constant\n"
		      + "\t that defines the smallest absolute value of significant change",
		      "alpha", 1, "-alpha <value>"));
	 ...
	 */
	public AbsoluteBR2QuerySelector(double g){
		super();
		setAlpha(g);
	}
	
	public AbsoluteBR2QuerySelector() {
		super();
	}
		...
		if ((numinstanceseachattribute[permutation[aiindex]]>0)&&(significantchange(alpha,pool,c))){
				   ...
				   while(true){
			        ...
			        if  (pool.instance(ii).isMissing(permutation[aiindex])) {
				        ...
				        //System.out.println("ii="+ii+",ai="+permutation[aiindex]);
	...
	private boolean significantchange(double alpha,Instances pool,NaiveBayesAttrUpdatable c){
		...
		if (sum_difference/numinstances > alpha){
			return true;}
		else {
			return false;}
	}
	...
	/*private int drawInst(Instances p, NaiveBayesAttrUpdatable c){
		return dk.blfw.Global.random.nextInt(p.numInstances());
	}*/
